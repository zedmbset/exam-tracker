# Exam Data And Report Architecture

This file is the shared reference for teammates who edit:
- `public/prompts/digitizePrompt.js`
- `public/prompts/doubleCheckPrompt.js`
- `src/server/reports/adminReportPdf.js`
- `src/server/reports/publicReportPdf.js`
- `src/server/reports/reportPdfShared.js`

Use it when working with AI models or when delegating design/content changes to another member.

## What This App Uses Today

The app now has two separate systems:

1. Prompt system
- `public/prompts/digitizePrompt.js`
- `public/prompts/doubleCheckPrompt.js`

2. Direct PDF report system
- `src/server/reports/adminReportPdf.js`
- `src/server/reports/publicReportPdf.js`
- `src/server/reports/reportPdfShared.js`

Admin and Public reports are no longer generated by prompts.
They are generated directly on the backend as real PDFs.

## Architecture Overview

### Prompt Flow
- `getExamContext()` in `public/exam.html`
- passes dynamic exam data into:
  - `generateDigitizePrompt(data)`
  - `generateDoubleCheckPromptFromContext(data, tsvData)`

`getExamContext()` now exposes canonical session data through:
- `examSession`
- `examSessionLabel`
- `examSessionShort`
- `periodLabelForTags`

Legacy `rotation` and `period` are still derived for compatibility, but they are no longer the source of truth.

`ExamSession` is stored in the sheet as a compact code:
- clinical: `R1-P1`
- preclinical: `S1`
- special: `RTRPG`, `SYNTH`

### Report PDF Flow
- `buildReportPayload(type)` in `public/exam.html`
- `POST /api/report-pdf` in `server.js`
- backend calls:
  - `buildAdminReportBuffer(data)` in `src/server/reports/adminReportPdf.js`
  - `buildPublicReportBuffer(data)` in `src/server/reports/publicReportPdf.js`
- both rely on shared helpers from `src/server/reports/reportPdfShared.js`

## File Responsibilities

### `public/prompts/digitizePrompt.js`
Owns the first AI prompt only.

Use this file when you want to improve:
- extraction instructions
- TSV formatting rules
- `[INCERTAIN: ...]` behavior
- association-question instructions
- Step 1 output quality

### `public/prompts/doubleCheckPrompt.js`
Owns the second AI prompt only.

Use this file when you want to improve:
- validation wording
- `[INCERTAIN]` audit logic
- final `VALIDATION PASSED` block format
- dispute review table
- Step 2 output contract

### `src/server/reports/adminReportPdf.js`
Owns the layout and content of the internal Admin PDF.

Use this file when you want to improve:
- admin report header
- workflow/status display
- internal metadata section
- digitization quality section
- admin-only links and presentation

Main sections inside the file:
- `drawAdminHeader(...)`
- `drawAdminIdentitySection(...)`
- `drawAdminQualitySection(...)`
- `drawAdminAccessSection(...)`
- `drawAdminFooter(...)`

### `src/server/reports/publicReportPdf.js`
Owns the layout and content of the student-facing Public PDF.

Use this file when you want to improve:
- public report header
- student summary card
- public statistics cards
- public access links
- overall visual style for student use

Main sections inside the file:
- `drawPublicHeader(...)`
- `drawPublicOverviewSection(...)`
- `drawPublicStatsSection(...)`
- `drawPublicAccessSection(...)`
- `drawPublicFooter(...)`

### `src/server/reports/reportPdfShared.js`
Owns shared PDF mechanics used by both PDFs.

Use this file when you want to improve:
- shared PDF engine behavior
- text encoding
- common color palette
- page size and margins
- common cards/sections/badges
- shared report context shaping
- shared file naming helpers

Main shared exports:
- `SimplePdf`
- `PDF_PAGE`
- `PDF_COLORS`
- `drawBadge(...)`
- `drawSectionTitle(...)`
- `drawInfoCard(...)`
- `drawLinksBlock(...)`
- `drawStatCard(...)`

## Session Model

The app now stores exam timing/grouping in one canonical sheet field: `ExamSession`.

Normalized variants:
- Clinical: rotation + period, for example `R1 + P2`
- Preclinical: semester only, for example `S1`
- Special: `RTRPG` or `SYNTH`

Derived display rules used by prompts and reports:
- Clinical display: `R1 • P2`
- Preclinical display: `S1`
- Special display: `Rattrapage` or `Synthese`

Tag/report period rules:
- Clinical exams use `P1`, `P2`, or `P3`
- Preclinical exams use `S1` or `S2`
- Special exams use `Rattrapage` or `Synthese`
- `buildReportContext(...)`
- `buildBaseReportFilename(...)`

Rule:
- if the change affects only Admin design, edit `src/server/reports/adminReportPdf.js`
- if the change affects only Public design, edit `src/server/reports/publicReportPdf.js`
- if the change affects both, edit `src/server/reports/reportPdfShared.js`

## Data Shape Used By Prompts And Reports

```js
{
  module: string,
  wilaya: string,
  year: string,
  level: string,
  period: string,
  rotation: string,
  examDate: string,
  membre: string,
  status: string,
  pdfUrl: string,
  affichagePdfUrl: string,
  csvUrl: string,
  quizLink: string,
  lang: string,
  nQst: number | string,
  hasCT: boolean,
  hasCas: boolean,
  hasComb: boolean,
  missingPos: string[],
  schemaQsts: string[],
  subcategoryMapEnabled: boolean,
  subcategories: object[]  // for Unit/Residanat exams
}
```

### Subcategories Structure

The `subcategories` array is only used for mapped exams (Unit 1-5 or Residanat).

Each subcategory object:
```js
{
  name: string,   // Subcategory name (e.g., "Anatomie", "Biochimie")
  range: string   // Question range (e.g., "1-20", "21-40")
}
```

Example for Unit 2:
```js
{
  subcategories: [
    { name: "Anatomie", range: "1-20" },
    { name: "Biochimie", range: "21-40" }
  ]
}
```

For normal exams (e.g., Dermatologie), `subcategoryMapEnabled` is `false` and the `subcategories` array is empty or not present.

### Exam Type Determination

The exam type is determined from the same values used by the live app:
- If `level === "7"` or `module === "Residanat"`: Exam type = `Residanat`
- Otherwise: Exam type = `Externat`

This affects:
- Tag construction (see TSV schema below)
- Whether subcategory UI is shown in Step 1

## TSV Column Derivation

The final TSV columns are derived from app context and PDF content:

| Column | Source | How It's Derived |
|---------|--------|-------------------|
| `Cas` | PDF | Copied from clinical case text in PDF |
| `Num` | PDF | Question number extracted from PDF |
| `Text` | PDF | Question stem extracted from PDF |
| `A`...`G` | PDF | Propositions extracted from PDF |
| `Correct` | PDF/CT | From Corrige Type if present, otherwise empty |
| `Exp` | Derived | Template based on `Correct` value |
| `Hint` | PDF | Association helper from PDF if applicable |
| `categoryId` | App context / derived mapping | Simple exam: row `categoryId`; composed exam: mapped from saved submodule ranges |
| `tagSuggere` | Derived | From `subcategories` mapping based on `Num` |
| `Year` | App context | Always `year` value |
| `Tag` | Derived | Built from context and row values, exported as comma-space text |

### Tag Construction Rules

The `Tag` column is a JSON array with exactly 4 elements:

**For Externat exams:**
```json
["Externat <Wilaya>", "<Period> <Year>", "No. <Num>", "Corrige type/propose"]
```

**For Residanat exams:**
```json
["Residanat <Wilaya>", "<tagSuggere> <Year>", "No. <Num>", "Corrige type/propose"]
```

Where:
- `<Wilaya>` comes from `wilaya` field
- `<Period>` comes from `period` field
- `<Year>` comes from `year` field
- `<Num>` comes from the question number
- `<tagSuggere>` comes from `subcategories` mapping (for Residanat only)
- Last element is `"Corrige type"` if `hasCT` is true, otherwise `"Corrige propose"`
- In the exported Excel/TSV, the array is displayed as comma-space text such as `Externat Constantine, 2024, No. 1, Corrigé type`

### Column Pruning Rules

After generating all rows for an exam:
1. Inspect all rows for that exam
2. Remove any column whose cells are empty for ALL rows
3. Preserve canonical order among remaining columns
4. Pruning is per exam, never per row

### Composed Exams

When the sheet row `categoryId` contains multiple IDs separated by commas, the app treats the exam as composed only if the combination matches one of the hardcoded definitions for that `Wilaya`.

In that case:
- Step 1 shows fixed draggable submodule rows
- submodule names are locked
- each row requires a question range
- the mapping is stored in `Tags.composedSubmodules`
- final exported questions receive the exact `categoryId` of the matching submodule range

Missing-question behavior:
- ranges are checked against real question numbers
- a missing question still belongs conceptually to its submodule range
- no exported row is created for the missing question
- later questions keep their original submodule/categoryId assignment

Commonly pruned columns:
- `F`, `G` (if no association questions)
- `Hint` (if no association questions)
- `tagSuggere` (if not a mapped exam)
- `Exp` (if all `Correct` values are empty)
## Field Meanings

- `module`: exam module name. Example: `Dermatologie`
- `wilaya`: location/faculty name. Example: `Setif`
- `year`: full stored year. Example: `2026`
- `level`: student level. Example: `6A`
- `period`: exam period. Example: `P1`
- `rotation`: rotation. Example: `R3`
- `examDate`: displayed exam date. Example: `10/9/2025`
- `membre`: readable summary derived from the `Membre` history cell, showing each participant Gmail and the steps they saved
- `status`: auto-maintained workflow status in the sheet. Values: `Completed`, `Pending`, `New Exam`, `Missing`, or empty for future/undated exams
- `pdfUrl`: Google Drive URL of the original exam PDF
- `affichagePdfUrl`: Google Drive URL of the affichage PDF when uploaded
- `csvUrl`: Google Drive URL of the final CSV
- `quizLink`: MBset quiz URL when available
- `lang`: exam language, usually `Francais`
- `nQst`: declared number of questions
- `hasCT`: whether Corrige Type exists
- `hasCas`: whether clinical cases exist
- `hasComb`: whether association/combination questions exist
- `missingPos`: missing or unreadable question numbers
- `schemaQsts`: question numbers containing image/schema/table content

## Common Derived Values

- `yearShort`:
  - `String(year).slice(-2)`
  - example: `2026 -> 26`

- `ref`:
  - `period + rotation`
  - example: `P1 + R3 -> P1R3`

- image prefix:
  - `${module}_${yearShort}${period}`
  - example: `Dermatologie_26P1`

- base report filename:
  - `${wilaya}_${level}_${module}_${yearShort}${period}${rotation}`
  - example: `Setif_6A_Dermatologie_26P1R3`

## Ready Example For Members

Example current exam context:

```js
{
  module: "Dermatologie",
  wilaya: "Setif",
  year: "2026",
  level: "6A",
  period: "P1",
  rotation: "R3",
  examDate: "10/9/2025",
  membre: "zeddrivepro@gmail.com (Step 1, Step 3)",
  status: "Completed",
  pdfUrl: "https://drive.google.com/...",
  affichagePdfUrl: "https://drive.google.com/...",
  csvUrl: "https://drive.google.com/...",
  quizLink: "https://app.mbset.com/quiz/start/...",
  lang: "Francais",
  nQst: 20,
  hasCT: true,
  hasCas: false,
  hasComb: false,
  missingPos: [],
  schemaQsts: []
}
```

Useful derived values from this example:

```js
yearShort = "26"
ref = "P1R3"
imagePrefix = "Dermatologie_26P1"
baseReportFilename = "Setif_6A_Dermatologie_26P1R3"
```

## How Members Should Use This With AI

### If the member improves prompts
Give the AI:
- the target file (`public/prompts/digitizePrompt.js` or `public/prompts/doubleCheckPrompt.js`)
- the output contract to preserve
- the example exam context above
- the exact behavior to improve

### If the member improves Admin PDF
Give the AI:
- `src/server/reports/adminReportPdf.js`
- `src/server/reports/reportPdfShared.js`
- this document
- instructions such as:
  - improve admin layout only
  - do not touch student/public content
  - keep exported function `buildAdminReportBuffer`

### If the member improves Public PDF
Give the AI:
- `src/server/reports/publicReportPdf.js`
- `src/server/reports/reportPdfShared.js`
- this document
- instructions such as:
  - improve student-facing design only
  - preserve current data fields
  - keep exported function `buildPublicReportBuffer`

## Safe Editing Rules

- Do not hardcode one exam unless you are intentionally building an example-only mockup.
- Keep data dynamic by using the provided `data` or `d` object.
- Do not move PDF layout logic back into `public/exam.html`.
- Do not duplicate shared PDF helpers into both report files.
- If you add a new field needed by prompts:
  - update `getExamContext()` in `public/exam.html`
- If you add a new field needed by report PDFs:
  - update `buildReportPayload()` in `public/exam.html`
- update `buildReportContext()` in `src/server/reports/reportPdfShared.js`

## Stable Function Names

Keep these names stable unless you also update all callers:
- `generateDigitizePrompt`
- `generateDoubleCheckPromptFromContext`
- `generateDoubleCheckPrompt`
- `buildAdminReportBuffer`
- `buildPublicReportBuffer`
- `buildReportContext`

## Quick Test Checklist After Editing

1. Hard refresh the exam page.
2. Confirm Step 1 prompt still renders and copies.
3. Confirm Step 2 prompt still renders and copies.
4. Generate Admin PDF and open the Drive URL.
5. Generate Public PDF and open the Drive URL.
6. Check accented text, layout alignment, and links.
7. Check that no file path or function name was broken.

