...
function parsePubLnkToken(token) {
  ...
  const optsString = raw.slice(start + 1, -1);
...
function parseActionString(rawValue) {
  ...
  for (const token of splitTopLevel(raw)) {
    flattened.push(...splitTopLevel(token));
  }
...
function serializeActionModel(model = {}) {
  const tokens = [];
  const grouped = Boolean(model.grouped);
  if (grouped) tokens.push('Grp');

  if (model.transferMode === 'posts') {
    tokens.push('Transfer[Posts]');
  } else if (model.transferMode === 'comments') {
    tokens.push('Transfer[Cmts]');
  }

  const pubLnkToken = serializePubLnk(model.pubLnk || {});
  if (pubLnkToken) tokens.push(pubLnkToken);

  if (!tokens.length) return '';
  if (!grouped || tokens.length === 1) return tokens.join(', ');
...
function summarizeActionModel(model = {}) {
  ...
  if (model.pubLnk?.enabled) bits.push('Pub link');
  if (model.pubLnk?.numSequence) bits.push('Numbered');
  if (model.pubLnk?.punctValue) bits.push(`Bullet ${model.pubLnk.punctValue}`);
  else if (model.pubLnk?.punctBlank) bits.push('No bullet');
  if (model.pubLnk?.joinUs) {
